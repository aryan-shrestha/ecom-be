...
class RemovePermissionFromRoleUseCase:
    """Use case for removing permission from a role."""
    ...
    async def execute(self, role_name: str, permission_code: str, user_id: uuid.UUID) -> bool:
        """
        Remove a permission from a role by name and code.
        ...
        async with self.uow:
            ...
            await self.uow.rbac.remove_permission_from_role(role_name=role_name, permission_code=permission_code)
            await self.uow.commit()

            await self.audit_log.log_event(
                event_type="permission.removed_from_role",
                user_id=str(user_id),
                details={
                    "role_name": role_name, 
