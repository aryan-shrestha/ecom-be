"""Clear all items from cart use case."""

from app.application.dto.cart_dto import CartDTO, ClearCartRequest
...
class ClearCartUseCase:
    """Remove all items from the actor's active cart."""

    def __init__(self, uow: UnitOfWork, clock: ClockPort) -> None:
        self.uow = uow
        self.clock = clock

    async def execute(self, request: ClearCartRequest) -> CartDTO:
        async with self.uow:
            ...
            await self.uow.carts.delete_all_items(cart.id)
            await self.uow.carts.update(cart)
            await self.uow.commit()

            return await build_cart_dto(self.uow, cart)

    async def _resolve_cart(self, request: ClearCartRequest):
