...
from app.application.dto.product_dto import UploadVariantImageRequest, VariantImageDTO
...
class UploadVariantImageUseCase:
    """Use case for uploading an image to a product variant via cloud storage."""
    ...
    async def execute(self, request: UploadVariantImageRequest) -> VariantImageDTO:
        """
        Upload image for product variant.
        ...
        async with self.uow:
            ...
            existing_images = await self.uow.products.get_images_for_variant(request.variant_id)
            ...
            try:
                upload_result = await self.file_storage.upload_image(
                    ...
                    folder=f"variants/{request.variant_id}",
            ...
            # Save image
            image = await self.uow.products.save_variant_image(image)
            await self.uow.commit()

            # Invalidate caches (variant belongs to product, so invalidate product caches)
            ...
            await self.audit_log.log_event(
                event_type="variant.image.uploaded",
